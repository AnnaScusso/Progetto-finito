...
Nomenclatura <- read.csv("C:/Users/Giovanni/Desktop/Prova/Nomenclatura.txt")
...
<br>
```{r}
piloti <- read_excel("C:/Users/Giovanni/Desktop/Motogp/Dataset_piloti.xlsx")
```
...
giri <- read_excel("C:/Users/Giovanni/Desktop/Motogp/Dataset_giri.xlsx")
giri[is.na(giri)]<-0
...
ggplot(freq2)+
  geom_bar( aes(x=freq2$N.Vittorie),stat="count")+
  coord_flip()+
  ggtitle("N. piloti in base alle frequenze di vittorie su gare disputate")+
  scale_x_continuous("Frequenze", breaks=seq(0,50,by=2))+
  theme_light()+
  ylab("N. piloti")
...
ggplot(freq2,aes(y=freq2$N.Vittorie, x=freq2$`N.gare disputate`, col=freq2$Rider))+
  ...
  labs(colour= "Pilota")+
  labs(title="N. Vittorie VS N. gare disputate")+  
...
```
<br>
Tra i big si nota che chi effettua tanti giri in prima posizione tendenzialmente ricopre una posizione prestigiosa in campionato.
...
```
<br>
Attraverso questo grafico viene dimostrata la tesi che il numero di giri veloci non è legata alla posizione in classifica: infatti si può notare che nel 2008 Stoner arrivato secondo in classifica campionato, ha fatto quasi il doppio (9) di giri veloci rispetto a Rossi (5 giri veloci) che ha vinto il campionato.
...
```{r}
vincitori2 <- read_excel("C:/Users/Giovanni/Desktop/Motogp/Dataset_vincitori2.0.xlsx")
```
<br>
...
```
<br>
Si nota che in certe piste c'è un predominio di una determinata casa costruttrice che continua da diverse stagioni, come si può notare per quanto riguarda Honda nel circuito 3 e 9. Questo predominio di una scuderia può essere legato a molti fattori, quali:
<br>
